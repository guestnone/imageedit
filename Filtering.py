...
from CustomConvKernelDialog import Ui_CustomConvKernelDialog
...
class ConvoluteKernelDialog(QDialog):
    def __init__(self):
        ...
        self.gaussKernel = np.array((
            [1, 2, 1],
            [2, 4, 2],
            [1, 2, 1]), dtype="int") * (1.0 / self.ui.gaussCoeffSpinBox.value())
    ...
    def getKernel(self):
        if self.ui.comboBox.currentText() == "Blur":
        ...
        elif self.ui.comboBox.currentText() == "Gauss":
            return np.array((
                [1, 2, 1],
                [2, 4, 2],
                [1, 2, 1]), dtype="int") * (1.0 / self.ui.gaussCoeffSpinBox.value())
            ...
            return np.array((
                [self.ui.kernelValueSpinBox1x1.value(), self.ui.kernelValueSpinBox1x2.value(),
                 self.ui.kernelValueSpinBox1x3.value()],
                [self.ui.kernelValueSpinBox2x1.value(), self.ui.kernelValueSpinBox2x2.value(),
                 self.ui.kernelValueSpinBox2x3.value()],
                [self.ui.kernelValueSpinBox3x1.value(), self.ui.kernelValueSpinBox3x2.value(),
                 self.ui.kernelValueSpinBox3x3.value()]), dtype="int")
    ...
    def onKernelTypeChange(self):
        ...
        if self.ui.comboBox.currentText() == "Blur":
            ...
            kernelToChange = self.gaussKernel
        else:
...
def processConvolute(image, kernel):
    ...
    for y in np.arange(padding, imageHeight + padding):
        for x in np.arange(padding, imageWidth + padding):
            regionOfInterest = grayImage[y:y + kernelWidth, x:x + kernelHeight]
    ...
    return cv2.cvtColor(finalImage, cv2.COLOR_GRAY2BGR)
...
class ConvoluteFiltering():
    ...
    def process(self, image, kernel):
        self.afterImage = processConvolute(image, kernel)


##################################################
### CUSTOM CONVOLUTION KERNEL FROM TEXT VALUES ###
##################################################

class CustomConvoluteKernelDialog(QDialog):
    def __init__(self):
        QWidget.__init__(self)
        self.ui = Ui_CustomConvKernelDialog()
        self.ui.setupUi(self)
        self.ui.buttonBox.accepted.connect(self.handleOK)
        self.ui.buttonBox.rejected.connect(self.handleCancel)
        self.isChange = False

    def processKernelInput(self):
        kValsStr = self.ui.plainTextEdit.toPlainText().split(sep=" ")
        if len(kValsStr) != (self.ui.spinBox.value() * self.ui.spinBox.value()):
            return False, "Incorrect number of values in input (is " + str(len(kValsStr)) + ", should be " + str(
                self.ui.spinBox.value() * self.ui.spinBox.value()) + ")"

        try:

            kernel = np.array(kValsStr).astype(np.int)
            self.kernel = kernel.reshape((self.ui.spinBox.value(), self.ui.spinBox.value()))
        except Exception as e:
            return False, "Error on parsing, " + str(e)

        return True, "OK!"

    @pyqtSlot()
    def handleOK(self):
        ok, errMsg = self.processKernelInput()
        if not ok:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Couldn't execute operation!")
            msg.setDetailedText("Reason: " + errMsg)
...
class CustomConvoluteFiltering():
    def __init__(self, image):
        self.dialog = CustomConvoluteKernelDialog()
        self.dialog.setModal(True)
        self.dialog.exec_()
        if self.dialog.getIfChanged():
            self.process(image, self.dialog.getKernel())
